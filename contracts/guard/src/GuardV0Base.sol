...
import {
    ...
    SEL_LIGHTER_WITHDRAW_PENDING
} from "./lib/LighterLib.sol";
...
abstract contract GuardV0Base is IGuard, Multicall {
    ...
    //
    // `assetIndex` is the USDC asset index on Lighter (ZkLighter.USDC_ASSET_INDEX()).
    // After calling this you must also:
    // 1. whitelistToken(usdc, notes)        — for approve()/transfer()
    // 2. allowReceiver(safeAddress, notes)  — Safe must be receiver for
    //    deposit._to / withdraw _owner (same requirement as whitelistGMX).
    //
    // NOTE: account registration is off-chain (EIP-712/EIP-1271 Safe signature) —
    // no guard call. changePubKey (trading-key rotation) is intentionally NOT
    // whitelisted here.
    //
    ...
    function whitelistLighter(
        ...
        allowApprovalDestination(zkLighter, notes);
        _whitelistToken(usdc, notes);
        LighterLib.whitelistLighter(zkLighter, assetIndex, notes);
    }

    function isAllowedLighter(address zkLighter) public view returns (bool) {
        return LighterLib.isAllowedLighter(zkLighter);
    }

    // Mirrors isAllowedGMXMarket — passes the guard's anyAsset joker through.
    function isAllowedLighterAssetIndex(uint16 assetIndex) public view returns (bool) {
