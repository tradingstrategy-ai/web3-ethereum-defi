...
import {HypercoreVaultLib, SEL_SEND_RAW_ACTION, SEL_CORE_DEPOSIT} from "./lib/HypercoreVault.sol";
...
abstract contract GuardV0Base is IGuard,  Multicall, SwapCowSwap  {
    ...
    function _validateCallInternal(
        ...
        bool hypercoreCheck = (selector == SEL_SEND_RAW_ACTION || selector == SEL_CORE_DEPOSIT);
        ...
        // --- Hypercore CoreWriter ---
            ...
            (uint24 actionId, address dest) = HypercoreVaultLib.validateAction(target, callData);
            if (actionId == 6) { // SPOT_SEND_ACTION
    ...
    function validate_swapExactTokensForTokens(bytes memory callData) internal view {
    ...
    function validate_ERC4626Deposit(address target, bytes memory callData) internal view {
        // This is no-op.
    ...
    function validate_ERC4626Withdraw(bytes memory callData) internal view {
    ...
    function validate_ERC4626Redeem(bytes memory callData) internal view {
        (, address receiver, ) = abi.decode(callData, (uint256, address, address));
        require(isAllowedReceiver(receiver), "Receiver not whitelisted");
    }
    ...
    function validate_cowSwapSettlement(bytes memory callData) internal view {
        (, address receiver, ) = abi.decode(callData, (uint256, address, address));
        require(isAllowedReceiver(receiver), "Receiver not whitelisted");
    }
    ...
    function whitelistCoreWriter(address coreWriter, address coreDepositWallet, string calldata notes) external onlyGuardOwner {
    ...
    function validate_orderlyDelegateSigner(bytes memory callData) internal view {
        // TODO: Implement validation
    }

    function validate_orderlyDeposit(bytes memory callData) internal view {
        // TODO: Implement validation
    }

    function validate_orderlyWithdraw(bytes memory callData) internal view {
        // TODO: Implement
    }
