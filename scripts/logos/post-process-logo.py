...
def process_logo_with_gemini(
    ...
    if invert == "light_to_dark":
        prompt_parts.extend([
            "3. INVERT COLOURS: This logo is designed for light backgrounds (dark-coloured logo). "
            "Invert the colours to create a version suitable for dark backgrounds. "
            "Make dark elements light/white and light elements dark. "
            "Preserve any brand colours where appropriate.",
            "",
        ])
        next_step = 4
    elif invert == "dark_to_light":
        prompt_parts.extend([
            "3. INVERT COLOURS: This logo is designed for dark backgrounds (light-coloured logo). "
            "Invert the colours to create a version suitable for light backgrounds. "
            "Make light/white elements dark and dark elements light. "
            "Preserve any brand colours where appropriate.",
            "",
        ])
        next_step = 4
...
def scale_image(input_path: Path, output_path: Path, size: int = 256) -> None:
    """Scale image to target size using high-quality resampling.
    ...
    logger.info("Scaling image to %dx%d: %s", size, size, input_path)
...
def process_logo(
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        scale_image(current_file, output_path, target_size)
