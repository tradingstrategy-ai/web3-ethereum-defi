...
path = Path.home() / ".tradingstrategy" / "vaults" / "vault-prices-1h.parquet"
price_df = pd.read_parquet(path, filters=[("chain", "==", chain_id)])

chains = price_df["chain"].unique()
print("We have data for chains:", list(chains))
...
assert len(price_df) > 0, f"No data found: {chain_id} in {path}"

print(f"Data is {price_df.index.min()} - {price_df.index.max()}, {len(price_df)} rows")

out_file = Path.home() / "Downloads" / f"chain-{chain_name}-raw-prices-{resample_period}.parquet"
