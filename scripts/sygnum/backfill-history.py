...
migration therefore writes only the one hardcoded metadata lead. It explicitly
...
from eth_defi.tokenised_fund.sygnum.constants import FILQ_A_ETHEREUM_ADDRESS, FILQ_A_ETHEREUM_FIRST_SEEN_AT, FILQ_A_ETHEREUM_FIRST_SEEN_AT_BLOCK, SYGNUM_ETHEREUM_CHAIN_ID
...
def create_detection() -> ERC4262VaultDetection:
    ...
    :return: Hardcoded non-event-derived FILQ-A detection.
    """

    return ERC4262VaultDetection(chain=SYGNUM_ETHEREUM_CHAIN_ID, address=FILQ_A_ETHEREUM_ADDRESS, first_seen_at_block=FILQ_A_ETHEREUM_FIRST_SEEN_AT_BLOCK, first_seen_at=FILQ_A_ETHEREUM_FIRST_SEEN_AT, features={ERC4626Feature.sygnum_like}, updated_at=native_datetime_utc_now(), deposit_count=0, redeem_count=0)


def create_lead() -> PotentialVaultMatch:
    """Create the one address-scoped discovery lead.

    :return: FILQ-A potential vault match.
    """

    return PotentialVaultMatch(chain=SYGNUM_ETHEREUM_CHAIN_ID, address=FILQ_A_ETHEREUM_ADDRESS, first_seen_at_block=FILQ_A_ETHEREUM_FIRST_SEEN_AT_BLOCK, first_seen_at=FILQ_A_ETHEREUM_FIRST_SEEN_AT, deposit_count=0, withdrawal_count=0)


def upsert_filq_metadata(vault_db: VaultDatabase, row: dict, end_block: int) -> None:
    """Upsert FILQ-A while restoring the shared Ethereum discovery cursor.

    :param vault_db: Existing shared vault database.
    :param row: Fresh FILQ-A metadata row.
    ...
    vault_db.update_leads_and_rows(chain_id=SYGNUM_ETHEREUM_CHAIN_ID, last_scanned_block=end_block, leads={FILQ_A_ETHEREUM_ADDRESS: create_lead()}, rows={spec: row})
    if had_cursor:
