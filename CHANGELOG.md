...
- fix: Repair four defects that together made GMX trading through a Lagoon vault impossible on Arbitrum Sepolia, each found by running the full deploy → deposit → open → close flow end to end against live testnet keepers. (1) `OrderArgumentParser._get_oracle_address_for_token()` kept a private copy of the testnet→mainnet oracle token map that had drifted from the canonical `TESTNET_TO_MAINNET_ORACLE_TOKENS`: it omitted Sepolia's regular USDC, so any order collateralised in it died with `KeyError` during price lookup, and it mapped Sepolia CRV to SOL's mainnet feed, silently pricing collateral off the wrong asset. (2) `Markets` accepted the REST `/markets` list verbatim; on Sepolia that advertises 16 markets against 10 actually registered in the DataStore, and ordering against a phantom market reverted with GMX's `EmptyMarket` *after* the guard had approved the call — the existing `IS_MARKET_DISABLED` check cannot catch these, since a flag never written reads as "enabled". (3) The `executionBuffer` constructor parameter was silently dropped when opening a position, because `_convert_ccxt_to_gmx_params()` never emits `execution_buffer`, so GMXTrading fell back to its own default and orders reverted with `InsufficientExecutionFee`; closing already forwarded it. (4) `size_usd` was honoured for opens but ignored for reduce-only closes, which sized from the token `amount` alone, so a USD-sized close computed to zero and failed outright (2026-07-28)
