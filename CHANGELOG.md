...
- fix: Persist the Anvil fork RPC cache by shutting Anvil down gracefully (SIGTERM + bounded wait for the flush, then SIGKILL fallback) instead of an immediate SIGKILL — Anvil only writes its `storage.json` fork cache on a graceful exit, so the previous hard kill meant the cache was never written and CI cold-fetched (and got rate-limited) every run; the existing `actions/cache` steps now warm across runs (2026-07-25)
...
- fix: Reduce vault-protocol CI RPC flakiness — cap test workers at 4, add a 15-minute per-test hard timeout, pin the shared Anvil fork pool to the bounded fail-fast upstream failover proxy (one attempt per provider, no dead-provider re-hammer), normalise flaky retries to one, and document a provider-redundancy + failure-diagnostics plan (2026-07-25)
- fix: Add RPC failure-mode diagnostics (out-of-credits / rate-limited / timeout, key-redacted provider names), a single-upstream-provider warning for fork test sessions, a repository-seedable Foundry fork RPC cache, documented `latest`-fork exemptions for chains without reliable archive state, and raising the GMX job cap to 30 minutes so the full suite (including slow order-execution tests) runs whenever GMX files change (2026-07-25)
