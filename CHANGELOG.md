...
- feat: Add 3Jane as a vault protocol and curator — recognise the USD3 (senior) and sUSD3 (junior) credit-market tranche stablecoins in `is_stablecoin_like()`, detect their ERC-4626 vaults via `HARDCODED_PROTOCOLS` as the new `ThreeJaneVault`/`threejane_like` protocol, and register 3Jane as a protocol-managed curator so its self-operated vaults attribute correctly; includes stablecoin/protocol/curator feed YAML wired through a single `3jane` source feeder, protocol + stablecoin metadata, and logos (2026-06-11)
