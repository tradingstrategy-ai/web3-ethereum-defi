...
- fix: Make the GMX oracle price query (`/signed_prices/latest`) fail over across all four API tiers (primary, backup, fallback, fallback-2) instead of stopping after the backup, and log which endpoint served the prices when the primary is down (2026-09-21)
...
- feat: Add YieldBasis Earn vault discovery, crvUSD and native-asset performance tracking, historical backfill tooling and reviewed Ethereum market metadata (2026-08-28)
...
- fix: Pin and cache GMX contract address resolution instead of fetching `gmx-io/gmx-synthetics` on every order, and refresh the vendored `Reader`, `GlvReader` and `ExchangeRouter` ABIs to GMX v2.2c — `get_contract_addresses()` previously fetched the `updates` branch per call and fell back to `main`, so the `ExchangeRouter` a live bot traded through changed when GMX shipped v2.2c and flip-flopped back to v2.2b whenever GitHub replied HTTP 429; against a Lagoon vault guard's fixed address allowlist that made every order-creating transaction revert with `execution reverted: Target not allowed`, exits included, leaving the bot unable to flatten risk. Addresses now come from `PINNED_CONTRACTS` (default `v2.2c`), overridable per call or via the `GMX_CONTRACT_RELEASE` environment variable, with `remote` restoring live resolution behind a 1-hour cache. Adds `assert_gmx_router_whitelisted()` so a router/allowlist mismatch fails once at startup rather than being discovered by broadcasting a reverting transaction per order, and corrects `GMX_ARBITRUM_ADDRESSES`, which held an `ExchangeRouter` older than either pinned release (2026-07-28)
- fix: Repair six defects that together made GMX trading through a Lagoon vault impossible on Arbitrum Sepolia, each found by running the full deploy → deposit → open → close flow end to end against live testnet keepers. (1) `NETWORK_CONTRACTS` still pointed at GMX's superseded Sepolia deployment: GMX shipped the same upgrade there that produced mainnet v2.2c, rotating `ExchangeRouter`, `Reader`, `GlvReader` and `OrderHandler`, but Sepolia has no entry in GMX's `docs/contracts.json` so the hand-maintained table silently drifted. The stale `Reader` failed to decode `getAccountOrders` — but only once an order was actually pending, since an empty array decodes fine against either struct layout, which is why it looked healthy until a live order existed. (2) `OrderArgumentParser._get_oracle_address_for_token()` kept a private copy of the testnet→mainnet oracle token map that had drifted from the canonical `TESTNET_TO_MAINNET_ORACLE_TOKENS`: it omitted Sepolia's regular USDC, so any order collateralised in it failed with `KeyError` during price lookup, and it mapped Sepolia CRV to SOL's mainnet feed, silently pricing collateral off the wrong asset. (3) `Markets` accepted the REST `/markets` list verbatim; on Sepolia that advertises 16 markets against 10 registered in the DataStore, and ordering against a phantom market reverted with GMX's `EmptyMarket` after the guard had already approved the call — `IS_MARKET_DISABLED` cannot catch these, since a flag never written reads as "enabled". (4) The `executionBuffer` constructor parameter was silently dropped when opening a position, because `_convert_ccxt_to_gmx_params()` never emits `execution_buffer`, so GMXTrading fell back to its own default and orders reverted with `InsufficientExecutionFee`. (5) `size_usd` was honoured for opens but ignored for reduce-only closes, which sized from the token `amount` alone, so a USD-sized close computed to zero; and once supported, such a close reported `filled=0` for a position closed in full on chain. (6) The Lagoon tutorial slept a fixed 30s instead of waiting for the keeper, so the close could run before the open was filled, report "already closed" having done nothing, and strand the position and its collateral. Also stops reporting a missing `gmxapi.ai` host as a failure: GMX runs no wallet-scoped REST service for Arbitrum Sepolia, so that tier now raises `GMXAPIChainUnsupported` and is skipped at DEBUG rather than warning on every call (2026-07-28)
...
- fix: Correct the 40acres fee assertions — the vault deliberately reports `0.0` lender fees (its 5% treasury cut comes from borrower rewards), not `None`, which the vault API reserves for "fee not exposed" (2026-07-25)
- test: Extend the committed Anvil fork RPC cache from the 10 canonical midnight blocks to all 39 fixed fork blocks the vault-protocol suite uses (1.7 MB), measured from a full local run — Monad is deliberately excluded because it can only fork the chain tip, so a committed block would never be hit again (2026-07-25)
- fix: Ship a committed ERC-20 / vault token cache for tests and install it as the vault default — vault token-address caching only engages for a `TokenDiskCache`, so the in-memory LRU default silently disabled it and started empty in every xdist worker, making vaults re-read token addresses and metadata over RPC on each cold fork; also pass the vault token cache through in the Upshift deposit analyser and Harvest `fetch_ftoken()` (2026-07-25)
- test: Commit the warm Anvil fork RPC cache seed (`eth_defi/testing/rpc_cache_seed/`, one small `storage.json` per canonical midnight block) and remove the per-run `actions/cache` fork-cache steps from the workflows — the committed seed is auto-applied by the session fixture, so every runner (GitHub Actions, other CI, local first runs) starts warm with no GitHub-Actions-cache dependency (2026-07-25)
- fix: Persist the Anvil fork RPC cache by shutting Anvil down gracefully (SIGTERM + bounded wait for the flush, then SIGKILL fallback) instead of an immediate SIGKILL — Anvil only writes its `storage.json` fork cache on a graceful exit, so the previous hard kill meant the cache was never written and CI cold-fetched (and got rate-limited) every run; this is what makes the committed cache seed generatable in the first place (2026-07-25)
...
- fix: Reduce vault-protocol CI RPC flakiness — cap test workers at 4, add a 5-minute per-test hard timeout under a 15-minute job cap, rely on the bounded fail-fast upstream failover proxy (one attempt per provider, no dead-provider re-hammer), normalise flaky retries to one, and document a provider-redundancy + failure-diagnostics plan (2026-07-25)
- fix: Add RPC failure-mode diagnostics (out-of-credits / rate-limited / timeout, key-redacted provider names and upstream vendor domain in fork-setup errors), a single-upstream-provider warning for fork test sessions, a repository-seedable Foundry fork RPC cache, documented `latest`-fork exemptions for chains without reliable archive state, and capped all CI workflow timeouts at 15 minutes while running the full GMX suite only when GMX files change (2026-07-25)

- feat: Add Xerberus Dendrogram backlinks to exported pool and protocol risk ratings (2026-07-28)
...
- fix: Close remaining cross-chain vault deposit/redemption simulation gaps — typed `VaultFlowUnavailable` preflights across the generic ERC-4626 flow, Anvil `force_settle` drivers for Ember and Gains, Lagoon settlement Safe approval/liquidity provisioning with honest synthetic-injection signalling, Accountable strategy minimum-deposit preflight, YieldNest and cSigma capacity preflights with decoded custom errors, a Plutus async-redemption manager, an Upshift deposit-limit hook, and a distinct `WhitelistingRequired` exception (2026-07-25)
...
- feat: Add read-only Superstate USTB tokenised-fund tracking, curator metadata and scoped historical backfill support (2026-07-17)
- feat: Add hardcoded Kinexys MONY tokenised-fund tracking with supply-only data and J.P. Morgan curator attribution (2026-07-17)
- feat: Add read-only Libeara CMTAT tokenised fund support for CUMIU and BELIF (2026-07-17)
- feat: Add Spiko USTBL tokenised-fund tracking with issuer Oracle NAV history and curator attribution (2026-07-17)
- feat: Add read-only Sygnum FILQ tokenised-fund support with supply-only history (2026-07-17)
- feat: Add read-only Theo thBILL tokenised-fund tracking and curator attribution (2026-07-17)
- feat: Add read-only Libeara ULTRA tokenised-fund tracking on Arbitrum with supply-only history (2026-07-17)
...
- feat: Add wstGBP vault support with hardcoded Ethereum discovery, NAV/share TVL history and Anvil-fork integration tests (2026-07-17)
...
- feat: Add broad new vault protocol coverage for [Aave v4 ERC-4626 spokes](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1119), [Aera](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1111), [Royco senior/junior tranches](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1063), [Ostium V1.5](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1066), [Mellow Core Vaults](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1163), [Kinexys JLTXX](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1188), [3Jane](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1102), [Domination Finance](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1055), [ForgeYields](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1032), [40acres](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/967), [CrystalClear](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1065), Inverse Finance, [Secured Finance](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/806) and [Liquid Royalty](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/837)

- feat: Expand native and perpetual vault data support across [Hyperliquid](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/946), [HyperCore](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/818), [GRVT and Lighter](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1089), and [Hibachi](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/969). This release adds high-frequency Hyperliquid collection, trade-history reconstruction, S3 archive backfill, HFT trader analytics, Google Sheets manual review sync, GRVT extended metadata, Lighter/GRVT curator attribution, Hibachi vault ingestion and richer vault metric exports

- feat: Add [Lighter integration for Lagoon and Guard deployments](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1140). Lighter deposits, withdrawals and pending-balance withdrawals can now be whitelisted in GuardV0 / TradingStrategyModuleV0, Safe-owned API keys can be registered through transaction-builder tooling, and the [Lagoon + Lighter mainnet lifecycle scripts](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1141) cover deposit, trading, withdrawal and redemption flows

- feat: Improve Lagoon, ERC-7540 and multichain deployment operations. [Lagoon async settlement tickets](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1110) can now be serialised and reconstructed across restarts, [pending deposit and redemption flow events](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1118) can be rediscovered from chain history, [multi-key asset-manager deployments](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/855) are supported, and [CCTP V2 event scanning](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1099) plus LI.FI gas-feeding utilities improve cross-chain vault operations

- feat: Add the [`eth_defi.currency_api` historical exchange-rate pipeline](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1158). Currency rates are scanned incrementally into DuckDB, scheduled as part of [`scan-vaults-all-chains`](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1161), exported and backed up with the vault artefacts, and the [default USD quote set](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1164) now includes EUR, GBP, JPY, AUD, BTC, ETH, SGD, TRY, CHF and CAD

- feat: Add [Core3 risk intelligence](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1053) to the vault scanner and JSON export pipeline. Core3 data is fetched into DuckDB, [mapped to vault protocol slugs](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1057), refreshed on a [24h scanner schedule](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1068), exported through R2 and [normalised into a top-level protocol-risk payload](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1075) for downstream consumers

- feat: Rework the vault export and scanner data pipeline. [Top-vaults JSON generation](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/945) is part of post-processing, public and private R2 uploads support [daily backups](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1026), [sticky append-biased export state](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1136) keeps previously accepted vaults visible through temporary data gaps, [sample data files](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/998) are published, and vault price pipelines now use [atomic writes](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1025), incremental cycle state and `written_at` timestamps

- feat: Expand [stablecoin](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/857) and denomination metadata. Stablecoin metadata now lives in YAML with logo and feed metadata, the scanner recognises [FIDD](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1107), [tGBP](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1157) and [several yield-bearing stablecoin derivatives](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1091), and [CoinGecko-backed denomination rate monitoring](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1143) can mark depegged denominations and blacklist affected vaults

- feat: Improve curator, protocol and feed metadata coverage. The release adds [manager-name curator attribution](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1123) for Euler, Morpho, Lagoon, GRVT and Lighter vaults; canonical feeder aliases; [richer short and long curator descriptions](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1081); many new curator records; and a [DuckDB-backed post feed pipeline](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/988) using X API v2, RSS and LinkedIn sources

- feat: Update the [GMX CCXT, Freqtrade and Lagoon integration](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1008) with liquidity-aware and collateral-aware market selection, shared async/sync market-loader disambiguation, REST / Reader / EventEmitter / Subsquid order-resolution fallbacks, persistent order keys across restarts, implemented fetch-trades support, [GMX Lagoon guard support for cancellation, update and claim multicalls](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1051), pre-flight rejection of markets that cannot accept the requested collateral, one bounded market-snapshot refresh before failing closed when collateral verification is unavailable, entry-price repricing for reduce-only closes so full closes do not leave dust positions, corrected order-state handling after restarts and partial data outages, non-market limit/stop orders protected from stale zombie cancellation, cache-miss timestamps based on block timestamps instead of block numbers, REST/on-chain market-cache revalidation, vanilla Freqtrade backtest behaviour for pairs with short candle history, and Sphinx-native GMX API docstrings

- fix: Improve scanner resilience against flaky data providers and rate limits. [HyperSync textual server rate limits](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1153) are retried, subprocess workers no longer storm RPC providers with [repeated chain ID checks](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1128), [HyperEVM scans](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1090) pin retries away from failing Goldsky consensus endpoints, [unsupported Morpho chains](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1132) are short-circuited, and transient RPC failures no longer wipe existing vault metadata

- fix: Improve transaction and accounting safety for live vault operations. [Nonce checks](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1135) now tolerate lagging fallback RPC nodes while preserving strict hot-wallet nonce ownership, [transaction receipt waits](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1097) are based on wall-clock confirmation time, [mainnet priority-fee floors](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1095) avoid slow inclusion, [HyperCore deposit verification](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1174) tolerates normal vault NAV drift, and exported vault JSON includes [token decimals](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1088) needed by consumers

- fix: Resolve several vault metric correctness issues. [Hyperliquid share-price reset cycles](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/804) are chain-linked, closed Hyperliquid vault deposits are detected correctly, [average utilisation](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/958) and deposit-closed reasons are exported, [period starts](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/869) are clamped to vault inception, [external TVL](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1046) can be stamped for offchain-only vaults, and [Lagoon false-positive detection](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/823) no longer crashes scans

- fix: Make feed and social collection more robust. [X list-member outages](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1086), suspended accounts, zero-result list timelines and [long note tweets](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1082) are handled without crashing scan loops or repeatedly retrying permanently bad accounts, while LinkedIn bridge failures can be auto-disabled with explicit metadata stamps

- refactor: Move shared HTTP retry support to [`eth_defi.logging_retry`](https://github.com/tradingstrategy-ai/web3-ethereum-defi/pull/1146) and normalise Core3 export placement. Shared retry logic is now reusable outside Velvet, and Core3 records moved from per-vault `other_data` into a top-level `core3_protocols` export keyed by protocol slug
