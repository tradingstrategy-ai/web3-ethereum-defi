...
- feat: Add Gauntlet as a supported vault protocol — detects Aera VaultV2 contracts via `adapterRegistry()` probe and MultiDepositorVault (Aera V3) via hardcoded addresses; reclassifies previously misidentified `morpho_v2_like` vaults as `gauntlet_like` since VaultV2 is Gauntlet/Aera infrastructure (2026-04-24)
