...
- feat: Add the `maintain-curators` skill that audits curator feeder YAML files for stale Twitter/LinkedIn/RSS/website error markers, decides whether each is real, and clears false positives, repoints renamed handles/feeds, or tombstones dead entities with a dated audit comment; first run cleared four X anti-bot false-positive markers (August Digital, B-CUBE.AI, WOO, SmarDex), kept six genuinely-dead `rss-dead-at` Medium feeds, and recorded `candleffect.com` as a dead website (NXDOMAIN) (2026-06-10)
