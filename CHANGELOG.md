...
- fix: Add RPC failure-mode diagnostics (out-of-credits / rate-limited / timeout, key-redacted provider names), a single-upstream-provider warning for fork test sessions, a repository-seedable Foundry fork RPC cache, a fixed-block conversion of the BNB decode-tx fork with documented `latest`-fork exemptions, and raising the GMX job cap to 30 minutes so the full suite (including slow order-execution tests) runs whenever GMX files change (2026-07-25)
