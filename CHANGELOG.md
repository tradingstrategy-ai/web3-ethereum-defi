...
- feat: Add Lighter API-key registration (`changePubKey`) tooling for a Safe-controlled (Lagoon vault) account — `eth_defi.lighter.pubkey` (validate / encode / propose / execute) and a CLI `scripts/lighter/lagoon-lighter-change-pubkey.py` that can propose to the Safe Transaction Service, execute for a single-owner Safe, or dry-run on a mainnet fork. This is a privileged Safe-owner action, deliberately outside the asset-manager guard whitelist. Includes Anvil-fork integration tests (registration is bound to `msg.sender`) (2026-06-25)
