...
- fix: GMX ccxt market resolution is now collateral-aware end-to-end, closing the `InvalidCollateralTokenForMarket` keeper-cancel incident on a live vault (2026-07-01/02). Root cause: the async GraphQL/REST market loaders lacked the sync loader's synthetic single-sided-pool disambiguation (`"{sym}2"` + exclusion), so a synthetic BTC pool collided with the real WBTC-USDC pool under one `BTC/USDC:USDC` symbol; Freqtrade's hourly market reload copied the collapsed async map onto the sync order-placing client, so live opens could target a pool that rejects USDC — the order shipped anyway (a soft-catch from #1008 claimed a swap-path fallback that is never actually built when `start_token == collateral`), burned gas, and died on-chain as a keeper cancel, tripping the circuit breaker for 60 minutes. Fixed in three layers: (1) both async loaders now mirror the sync loader's wstETH special-case + synthetic-pool exclusion, the markets disk cache is versioned (`_v2`) so pre-fix poisoned rows are orphaned rather than served for up to the 1h TTL after upgrade, and all four loaders skip malformed records with missing long/short token fields instead of mislabelling them; (2) `_resolve_market_info`'s default branch — the only market-selection logic that actually runs on the live ccxt order path — now keeps the mapped pool only if it accepts the order's collateral, otherwise selects a same-index sibling that does, defaulting to USDC (the deepest-liquidity pools); (3) `OrderArgumentParser` raises a new `InvalidCollateralForMarketError` pre-flight when a market definitively rejects the collateral and no swap leg can be built, instead of shipping a doomed order; when `start_token == collateral` and the verdict is indeterminate (market absent from a stale snapshot), it forces one bounded `Markets` cache refresh and, if still unverifiable, fails closed with `CollateralVerificationUnavailableError` rather than shipping an unverified empty-swap-path order that can only keeper-cancel; the Arbitrum BTC-collateral special case now runs the same validation as every other collateral (it previously returned early and bypassed the guard) (#1178) (2026-07-03)

- fix: GMX reduce-only position closes no longer leave ~1% token dust. `_convert_ccxt_to_gmx_params` priced the requested close at the *current* market price and compared it against the on-chain `sizeInUsd` (an entry-priced invariant) to decide full-vs-partial; whenever price had moved since entry (a short in profit / long in loss) a genuine 100%-of-tokens close fell below the 99.5% full-close tolerance, was submitted as a partial USD decrease, and GMX reduced the position only proportionally — leaving token dust ≈ the price-move percentage (the source of the recurring `position_reconciliation` ghost warnings). The close request is now repriced at the position's entry price (`close_amount × entry_price`, decimals-free, with a defensive fallback to current price), so a full-token close resolves to ~= `sizeInUsd` regardless of price move and the existing raw-int full-close substitution fires; genuine partials (DCA `sub_trade_amt`, same-pair sibling logical trades) are unaffected (#1178) (2026-07-03)
...
- fix: Stop the feed post scanner retrying suspended X accounts on every list sync. Suspended accounts that still resolve to a user ID but fail `add_list_member` with "You cannot add a member that is suspended" are now treated as non-retryable account-state failures, stamped with the existing `twitter-handle-resolved-unknown-at` feeder marker from the scanner, and excluded from subsequent sync cycles; also repoint SmarDex to its current `every_thing` handle and disable suspended Lybra Finance eUSD Twitter collection (2026-06-28)
...
- feat: Add Lighter (zk-rollup perps DEX on Ethereum L1) deposit/withdraw whitelisting to GuardV0 / TradingStrategyModuleV0 — a new `LighterLib` Forge library validates `deposit` / `withdraw` / `withdrawPendingBalance` against the `ZkLighter` L1 contract (receiver checks via `isAllowedReceiver`, USDC asset-index allow-set with `anyAsset` joker, following the GmxLib pattern), wired through the Lagoon vault deployment flow and `eth_defi.lighter.deployment.setup_lighter_whitelisting`. Currently USDC-only. Includes a manual end-to-end tutorial `scripts/lagoon/lagoon-lighter-example.py` (SIMULATE mode on an Ethereum mainnet fork) covering vault deploy → vault deposit → Lighter L1 deposit, with off-chain trading documented (2026-06-25)

- feat: GMX Lagoon guard now whitelists `cancelOrder`, `updateOrder` and the `claim*` functions (`claimFundingFees` / `claimCollateral` / `claimAffiliateRewards`) inside `ExchangeRouter.multicall`, so vaults can cancel/modify GMX orders and claim rewards instead of reverting with "GMX: Unknown function in multicall". Claim receivers are validated against `isAllowedReceiver()`. Fixes [#1050](https://github.com/tradingstrategy-ai/web3-ethereum-defi/issues/1050) (2026-06-02)
- fix: Stop a lagging fallback RPC node from crashing the strict nonce check — `check_nonce_mismatch()` keeps strict equality (our hot wallet counter solely owns the nonce) but, when the first read disagrees, re-samples every fallback node and trusts the most up-to-date one (highest block number), retrying so a node one transaction behind on an eventually-consistent multi-node RPC (e.g. HyperEVM behind Alchemy/Goldsky/dRPC) no longer raises a false `NonceMismatch`. On a genuine, persisting desync it still raises, now with a friendly per-node block/nonce diagnostic so the lagging node is obvious. Also adds deterministic local batch validation (duplicate nonce in a batch raises, gaps warn). Fixes the HyperCore sequential-trade executor crash where a signed nonce 1403 was rejected against a stale 1402 read (2026-06-24)

- feat: Add sticky append-biased vault JSON export state so vaults that once pass the production peak TVL filter remain in `top_vaults_by_chain.json` across temporary metric or metadata gaps, with a shared `vault-export-state.json` state file, structural blacklist/invalid-record suppression, stale annotations, atomic writes and R2 data backups for the state file; move the exporter implementation to `eth_defi.vault.top_vaults_json` so post-processing can call it through normal package imports (2026-06-24)
...
- feat: Complete the generic async vault settlement interface for ERC-7540 Lagoon vaults — `ERC7540DepositManager` now implements ticket `serialize_deposit_ticket()`/`serialize_redemption_ticket()`/`reconstruct_deposit_ticket()`/`reconstruct_redemption_ticket()` (persisting the ERC-7540 `request_id`) and `get_deposit_request_status()`/`get_redemption_request_status()`, so settlement pollers like trade-executor's `check_and_resolve_vault_settlements()` survive process restarts; the status methods check the request-specific `claimableDepositRequest`/`claimableRedeemRequest` before the aggregate `pendingDepositRequest(0, owner)` query so an already-claimable request is not misreported as pending while another request is outstanding. Raw amounts in serialised tickets are now stored as strings because 18-decimal share counts exceed the JavaScript safe-integer limit enforced by trade-executor state files (readers accept the legacy int form via `int()`). `LagoonVault.request_redeem()` gains a `check_enough_token` flag so a request object can be rebuilt to parse an already-broadcast `requestRedeem()` transaction after the shares have moved into the vault escrow (2026-06-12)
...
- feat: Add 3Jane as a vault protocol and protocol-managed curator — detect its USD3 (senior) and sUSD3 (junior) credit-market ERC-4626 vaults via `HARDCODED_PROTOCOLS` as the new `ThreeJaneVault`/`threejane_like` protocol (with the junior-tranche one-month `SUSD3_LOCK_DURATION` redemption lock), and register 3Jane as a protocol-managed curator so its self-operated vaults attribute correctly; includes protocol + curator feed YAML aliased through a single `3jane` source feeder, vault protocol metadata, a docs page, and logos. The USD3/sUSD3 stablecoin recognition (`is_stablecoin_like()` and stablecoin metadata) already landed separately on master (2026-06-11)
...
- fix: Make `wait_for_transaction_receipt_robust()` wait wall-clock time instead of a fixed block count — new `confirmation_block_time` argument (default `DEFAULT_CONFIRMATION_BLOCK_TIME` = 25 seconds) is converted to a per-chain block count via the new `get_evm_block_time(chain_id, block_number)` helper and combined as `max(time_based_blocks, confirmation_block_count)`. A fixed 2-block confirmation is ~24 s on Ethereum but only ~0.5 s on Arbitrum, which let an `eth_call` right after a confirmed `approve()` read a stale `allowance()` of 0 from a lagging backend behind a load-balanced RPC endpoint, aborting a Lagoon vault first deposit (2026-06-10). `get_evm_block_time()` returns `None` for unknown chains so callers degrade gracefully; its `block_number` argument is reserved for future dynamic block time resolution. The `fund_lagoon_vault()` settle wait opts out with `confirmation_block_time=0` because the `maxDeposit()` poll loop that follows already tolerates stale reads (2026-06-10)

- fix: Stop Lagoon multichain deployments aborting on slow Ethereum mainnet transaction inclusion — `estimate_gas_price()` now floors the RPC suggested priority fee to a documented per-chain minimum (`MIN_PRIORITY_FEE_PER_CHAIN`: 1 gwei on Ethereum mainnet, 30 gwei on Polygon, no floor for rollups), overridable via the new `min_priority_fee` argument, so transactions no longer sit unmined with an effectively zero tip; Safe and Lagoon vault deployment helpers wait 5 minutes (`DEFAULT_TX_CONFIRMATION_TIMEOUT`) instead of 120 seconds for transaction confirmation; and `add_new_safe_owners()` no longer aborts on a receipt timeout when the owner addition or threshold change confirmed late on chain (2026-06-10)

- feat: Stamp the vault scanner Docker image with its git revision and embed it in the top-vaults JSON export — `Dockerfile.vault-scanner` now writes `GIT_VERSION_TAG/COMMIT_MESSAGE/VERSION_HASH.txt` files at build time (trade-executor pattern), the new `eth_defi.version_info.VersionInfo` helper reads them back at runtime, the export gains a typed top-level `metadata.version` dict and the scanner logs its version at startup, so stale exporter deployments are diagnosable from the JSON artifact alone; also thread `feed_db_path` explicitly through `run() → run_scan_tick() → run_post_processing() → export_top_vaults_json()` with a new `resolve_feed_database_path()` resolver (`FEED_DB_PATH` → `DB_PATH` → default, mirroring the post scanner) so curator feed entries can no longer silently vanish when the pipeline data dir and feed database location diverge (2026-06-10)

- fix: GuardV0Base — `allowDelegationApprovalDestination()` now emits `DelegationApprovalDestinationApproved` instead of the wrong `ApprovalDestinationApproved` event, so the off-chain guard config scanner classifies delegation approval destinations correctly; also corrected the stale `anyAsset` security comment which wrongly claimed SwapRouter02 swaps skip recipient validation (the swap recipient is always checked against `allowedReceivers`; only per-token swap path validation is skipped) (2026-06-10)

- feat: Add the `maintain-curators` skill that audits curator feeder YAML files for stale Twitter/LinkedIn/RSS/website error markers, decides whether each is real, and clears false positives, repoints renamed handles/feeds, or tombstones dead entities with a dated audit comment; the skill sharply separates `twitter-dead-at` (recency, needs a concrete post within the 180-day `death_detection_days` cutoff to clear) from `twitter-handle-resolved-unknown-at` (handle validity, refuted by a live profile); first run cleared SmarDex's handle-unknown marker, retained the `twitter-dead-at` markers on August Digital / B-CUBE.AI / WOO (recency unverifiable or genuinely stale) with documented audit comments, kept six genuinely-dead `rss-dead-at` Medium feeds, and recorded `candleffect.com` as a dead website (NXDOMAIN) (2026-06-10)
...
- feat: Add Domination Finance dfUSDC vault protocol support with hardcoded Base address detection, metadata, logos and Sphinx docs (2026-06-03)
...
- feat(gmx/freqtrade): on-chain `fetch_positions` reconciliation inside `Gmx.fetch_order`. When the parent CCXT resolver returns `status="open"` for a non-market order (limit / stopLoss / take_profit / stop), the wrapper now cross-checks against `_api.fetch_positions([pair])`. If a matching position exists (same pair + side + contracts within 0.5%), the order is flipped to `status="closed"` with `filled=amount`. This catches the BONK / SHIB stuck-order pathology where the primary resolver was blinded by 818×404 + 818×400 Subsquid errors. Replaces a planned periodic watchdog with an inline lazy check — zero overhead when fills are detected normally, triggers exactly when freqtrade asks for fill state. Sync only; freqtrade's main loop is sync. See `docs/claude-plans/2026-05-14-gmx-market-catalog-fix.md` and tradingstrategy-ai/gmx-strategies#67 (2026-05-14)

- fix(gmx/ccxt): reorder `_resolve_order_from_sources` tier ladder — REST v2 (`/orders` + `/positions`) is now Tier A, on-chain Reader is Tier B, EventEmitter log scan is Tier C, Subsquid `tradeActions` demoted to Tier D (last-resort). Driven by 1,636 Subsquid 4xx errors in 2026-05-14 production logs that blinded the resolver to BONK / SHIB fills when it was Tier A. Plus structured logging on the final WARNING when every tier misses: `tier_errors={...}` dict captures per-tier failure mode (`extra={}` for log aggregators), making "all tiers exhausted" diagnosable without reproducing the failure. See tradingstrategy-ai/gmx-strategies#67 (2026-05-14)

- feat(gmx/ccxt): disk-persisted `OrderKeyCache` at `~/.cache/eth_defi/gmx/order_keys_{chain_id}_{wallet_lower}.json`. The previous in-memory-only `self._orders` dict was wiped on every bot restart, dropping `order_key` for live limit orders and producing the BONK / SHIB stuck-trade events (1,773 + 1,785 occurrences in 2026-05-14 logs). New `OrderKeyRecord` dataclass + `OrderKeyCache` class with atomic writes (`.tmp` + rename), case-insensitive `tx_hash` lookups, lazy load, 30-day stale-entry prune, and best-effort persistence (disk failure downgrades to memory-only with WARNING). Integration into `CcxtExchange` write/lookup sites is queued as a follow-up; the module is standalone-tested (16/16 green). See tradingstrategy-ai/gmx-strategies#67 (2026-05-14)

- feat(gmx/catalog): new `eth_defi.gmx.core.market_catalog` module — liquidity-aware GMX V2 market catalog replacing the legacy first-match `find_all_market_keys_by_index_address` sort. Exposes `MarketEntry` frozen dataclass (with `is_synthetic` + `accepts_collateral` predicates), `enumerate_markets` (REST primary + on-chain Reader fallback + defensive `SYMBOL_NORMALISE`), `augment_with_liquidity` (on-chain multicall via `GetAvailableLiquidity` + `GetOpenInterest`, no Subsquid), `MarketCatalog` class with 5-min memory + 24-h disk TTL caching, single-flight lock, atomic disk writes. Selection via `MarketCatalog.pick_market(base, selection, explicit_market_key)` supports three strategies: `USDC_PAIRED` (default — standard `<base>-USDC` pool, WBTC-USDC for BTC, WETH-USDC for ETH), `HIGHEST_LIQUIDITY` (pure TVL ranking), `EXPLICIT` (caller-supplied `market_key`). USDC_PAIRED falls back to HIGHEST_LIQUIDITY when no USDC pool exists for the base. 28 unit tests cover construction, enumeration, augmentation degradation, cache TTL semantics, corrupt-file resilience, and all three selection strategies. See tradingstrategy-ai/gmx-strategies#67 (2026-05-14)

- fix(gmx/order): `OrderArgumentParser._handle_missing_market_key` now prefers USDC-paired pools when multiple markets share an index token and no explicit `collateral_symbol` matches. Previously the fallback ("use first match") could deterministically land BTC orders on the synthetic tBTC-tBTC pool, then fail with `Not a valid collateral for selected market!` for USDC orders. Plus: `_check_if_valid_collateral_for_market` no longer raises on mismatch — returns `False` (with INFO log) so the GMX router's `swap_path` auto-conversion can handle non-matching collateral instead of hard-failing the order. Caller always assigns `parameters_dict["collateral_address"]`. See tradingstrategy-ai/gmx-strategies#67 (2026-05-14)

- feat(gmx/symbols): add `kPEPE → PEPE` and `kFLOKI → FLOKI` entries to `SYMBOL_NORMALISE`, preventing future fill-detection breakage when those pairs join the universe. Plus new `tests/gmx/test_symbols.py` pinning the full map's coverage (kBONK, kSHIB, kPEPE, kFLOKI, XAUT.v2) and the lowercase invariant for `DEPRECATED_MARKET_TOKENS` lookups. (2026-05-14)
...
- fix: GMX CCXT adapter — `GMX.fetch_order` cache-miss path now uses the block's epoch-millisecond timestamp (`block.timestamp * 1000`) instead of `block.number * 1000`. The previous behaviour made the synthetic order's `timestamp` ~3 orders of magnitude too small; the freqtrade wrapper's age check then saw apparent ages of ~41 years and zombie-cancelled limit orders in memory on every bot restart, producing orphan on-chain positions. Fixed in both the sync and async adapters (3 sites each), via a new `_block_timestamp_ms` helper that returns `None` on RPC failure to keep `fetch_order` non-fatal. See tradingstrategy-ai/gmx-strategies#67 (2026-05-11)
