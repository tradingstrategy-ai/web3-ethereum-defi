...
- ``DERIVE_WALLET_ADDRESS``: Derive wallet address (optional, for verification)
"""
...
@pytest.fixture(scope="module")
def expected_wallet_address():
    return os.environ.get("DERIVE_WALLET_ADDRESS")
...
def test_derive_wallet_matches_expected(derive_wallet_address, expected_wallet_address):
    """Verify the resolved wallet matches the expected address from env."""
    if not expected_wallet_address:
        pytest.skip("DERIVE_WALLET_ADDRESS not set")

    assert derive_wallet_address.lower() == expected_wallet_address.lower(), f"Resolved wallet {derive_wallet_address} does not match expected {expected_wallet_address}"
