# Utility script to mock keeper actions to emulate off-chain activities to confirm balance change after swap
import json
import os
...
from gmx_python_sdk.scripts.v2.gmx_utils import create_hash_string, get_datastore_contract, get_reader_contract
...
from gmx_python_sdk.scripts.v2.utils.keys import MAX_ORACLE_REF_PRICE_DEVIATION_FACTOR, IS_ORACLE_PROVIDER_ENABLED
...
def set_opt_code(w3: Web3, bytecode=None, contract_address=None):
    ...
    # This check is obsoltete for anvil forks so comment this out
    # else:
...
def update_swap_utils(w3: Web3):
    """
    Update the bytecode of a deployed library

    Args:
        w3: Web3 instance
    """
    # 1. Get the path to your compiled library bytecode
    abis_path = os.path.dirname(os.path.abspath(__file__))
    artifacts_path = Path(f"{abis_path}/mock_abis/SwapUtils.json")
    swap_utils_address = to_checksum_address("0x5Ef80E5783FB1dE6C156fA5fce88E5ee28514153")

    # 2. Load library ABI and bytecode from compilation artifacts
    with open(artifacts_path) as f:
        contract_json = json.load(f)
        abi = contract_json["abi"]
        # For libraries, we need the deployedBytecode, not the bytecode with constructor
        bytecode = contract_json["deployedBytecode"]

    # 3. Verify this is the correct library before updating
    old_bytecode = w3.eth.get_code(swap_utils_address)
    print(f"Original library bytecode size: {len(old_bytecode)} bytes")

    # 4. Update the bytecode using anvil_setCode
    set_opt_code(w3, bytecode, swap_utils_address)

    new_bytecode = w3.eth.get_code(swap_utils_address)
    print(f"Updated library bytecode size: {len(new_bytecode)} bytes")
...
def execute_order(config, connection, order_key, deployed_oracle_address, initial_token_address, target_token_address, logger=None, overrides=None):
    ...
    Args:
        ...
        initial_token_address: input token for swap,
        target_token_address: output token after swap,
...
def emulate_keepers(config, start_token: str, output_token: str, w3: Web3, recipient_address, initial_token_address, target_token_address):
    chain_id: int = 42161
    # tokens: dict = {
    #     "USDC": USDC_NATIVE_TOKEN[chain_id],
    ...
    #     "ETH": WRAPPED_NATIVE_TOKEN[chain_id]
    # }

    initial_token_symbol: str = start_token
    target_token_symbol: str = output_token
    ...
    # print(f"Recipient {target_token_address} balance before: {balance_decimal:.18f} {target_symbol}")
    ...
    update_swap_utils(w3)

    try:
        data_store = get_datastore_contract(config)

        assert ORDER_LIST.hex() == "0x86f7cfd5d8f8404e5145c91bebb8484657420159dabd0753d6a59f3de3f7b8c1", f"Order list mismatch, EXPECTED: {ORDER_LIST.hex()}"
        ...
        custom_oracle_provider = deployed_oracle_address
        ...
        another_key: str = "0x19532fed75fd7a66414dbc262ba18d9c0c8957aaa87cd1060ecdb785aa0dc18d"
        another_large_value: int = 631945365492
        data_store.functions.setUint(another_key, another_large_value).transact({"from": controller})
        value = data_store.functions.getUint(another_key).call()

        assert value == another_large_value, f"Value should be {another_large_value}"
        ...
        max_price: int = int(oracle_prices[initial_token_address]["maxPriceFull"])
        ...
        execute_order(config=config, connection=w3, order_key=order_key, initial_token_address=initial_token_address, target_token_address=target_token_address, deployed_oracle_address=deployed_oracle_address, overrides=overrides)
        ...
        #
        # # Format to avoid scientific notation and show proper decimal places
        # # print(f"Recipient {target_token_address} balance after swap: {balance_decimal:.18f} {target_symbol}")
        # # print(f"Change in {target_token_address} balance: {Decimal((target_balance_after - target_balance_before) / 10 ** target_decimals):.18f}")
