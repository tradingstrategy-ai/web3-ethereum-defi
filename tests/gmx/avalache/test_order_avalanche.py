"""
Tests for GMXOrderManager on Avalanche network.

This test suite tests the functionality of the GMXOrderManager class
when connected to the Avalanche network using an Anvil fork.
"""
import pytest
...
def test_initialization(gmx_config_avalanche):
    """
    Test that the order manager initializes correctly with avalanche config.
    """
    order_manager = GMXOrderManager(gmx_config_avalanche)
    assert order_manager.config == gmx_config_avalanche
    assert order_manager.config.get_chain().lower() == "avalanche"


def test_get_open_positions_with_known_address(gmx_config_avalanche, account_with_positions_avalanche):
    """
    ...
    positions = order_manager.get_open_positions(address=account_with_positions_avalanche)
...
def test_fail_close_position_parameter_validation(order_manager_avalanche):
    """
    ...
    with pytest.raises(ValueError):
        order_manager_avalanche.close_position(parameters=invalid_params, debug_mode=True)


def test_close_position_creates_valid_order(order_manager_avalanche):
    """
    Test that close_position creates a valid decrease order.

    This test verifies order creation with debug_mode=True.
    """
    # Valid parameters for closing an AVAX position
    valid_params = {"chain": "avalanche", "index_token_symbol": "AVAX", "collateral_token_symbol": "AVAX", "start_token_symbol": "AVAX", "is_long": True, "size_delta_usd": 10, "initial_collateral_delta": 2, "slippage_percent": 0.05}  # Close ~$40 worth  # Remove 2 AVAX  # 5% slippage

    # Create order in debug mode
    order = order_manager_avalanche.close_position(parameters=valid_params, debug_mode=False)
...
def test_fail_close_position_by_key_raises_for_invalid_key(order_manager_avalanche):
    """
    ...
    with pytest.raises(ValueError, match="Position with key .* not found"):
        order_manager_avalanche.close_position_by_key(position_key="NON_EXISTENT_KEY", out_token_symbol="AVAX", debug_mode=False)


def test_fail_close_position_by_key_invalid_format(order_manager_avalanche):
    """
    Test that close_position_by_key validates key format.
    """
    # Invalid key format (no underscore)
    with pytest.raises(ValueError, match="Position with key .* not found"):
        order_manager_avalanche.close_position_by_key(position_key="INVALID_FORMAT_WITH_NO_PROPER_STRUCTURE", out_token_symbol="AVAX", debug_mode=False)


def test_close_position_chain_default(order_manager_avalanche):
    """
    ...
    # Create order in debug mode
    order = order_manager_avalanche.close_position(parameters=params, debug_mode=False)
...
def test_close_position_partial_amount(order_manager_avalanche):
    """
    Test creating an order to close a partial amount of a position.
    """
    # Parameters for closing a small amount
    params = {"chain": "avalanche", "index_token_symbol": "AVAX", "collateral_token_symbol": "AVAX", "start_token_symbol": "AVAX", "is_long": True, "size_delta_usd": 20, "initial_collateral_delta": 2, "slippage_percent": 0.03}  # Only close $20 worth  # Remove only 2 AVAX  # 3% slippage
...
def test_full_workflow_with_existing_positions(order_manager_avalanche, account_with_positions_avalanche):
    """
    ...
    positions = order_manager_avalanche.get_open_positions(address=account_with_positions_avalanche)
    ...
    params = {"chain": "avalanche", "index_token_symbol": market_symbol, "collateral_token_symbol": market_symbol, "start_token_symbol": market_symbol, "is_long": is_long, "size_delta_usd": min(5, position["position_size"] * 0.01), "initial_collateral_delta": position["inital_collateral_amount"] * 0.01, "slippage_percent": 0.05}  # Close 1% or $5, whichever is smaller  # Remove 1% of collateral  # 5% slippage
    ...
    order2 = order_manager_avalanche.close_position_by_key(position_key=position_key, out_token_symbol=out_token_symbol, amount_of_position_to_close=0.01, amount_of_collateral_to_remove=0.01, slippage_percent=0.05, debug_mode=False, address=account_with_positions_avalanche)  # Close 1%  # Remove 1% of collateral
