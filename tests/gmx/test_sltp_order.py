...
def test_standalone_long_with_take_profit(isolated_fork_env, execution_buffer):
    ...
    if usdc_token:
        usdc_contract = env.web3.eth.contract(address=usdc_token.address, abi=[{"constant": True, "inputs": [{"name": "_owner", "type": "address"}], "name": "balanceOf", "outputs": [{"name": "balance", "type": "uint256"}], "type": "function"}])
