...
The post-scan stamp_external_tvl() function updates the total_assets column
...
3. Verify total_assets is set on the existing row (not a new row)
4. Verify the value feeds through to period metrics for ranking eligibility
"""
...
def _make_mock_vault(address: str, tvl_usd: Decimal | None, chain_id: int = 1, historical_supported: bool = False) -> MagicMock:
...
def test_stamp_updates_latest_row():
    """Stamp external TVL onto total_assets of the latest existing row.

    1. Create a parquet with two price rows for a ForgeYields vault (different timestamps)
    2. Run stamp_external_tvl
    3. Verify total_assets is set on the latest row only
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ta_values = result.column("total_assets").to_pylist()
        # First row (older) should still be NaN
        assert math.isnan(ta_values[0])
        # Second row (latest) should have the stamped TVL
        assert ta_values[1] == pytest.approx(1085984.11)


def test_stamp_preserves_share_price():
    """Verify that stamping total_assets does not corrupt existing share_price.

    1. Create a parquet row with a valid share_price
    2. Stamp total_assets via external TVL
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        forge_vault = _make_mock_vault(addr, Decimal("500000.00"))
        stamp_external_tvl(output_fname=parquet_path, vaults=[forge_vault])

        # 3. Verify share_price preserved, total_assets set
        result = pq.read_table(str(parquet_path))
        assert result.column("share_price").to_pylist()[0] == pytest.approx(1.085)
        assert result.column("total_assets").to_pylist()[0] == pytest.approx(500000.00)
...
def test_stamp_keys_by_chain_and_address():
    ...
    3. Verify only the chain 1 row gets total_assets, chain 42161 stays NaN
    """
    addr = "0x943109dc7c950da4592d85ebd4cfed007af64670"
    ts = datetime.datetime(2026, 5, 28, 12, 0, 0)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        assert ta_values[chain1_idx] == pytest.approx(1000000.00)

        # Chain 42161 row should still be NaN
        chain42161_idx = chains.index(42161)
        assert math.isnan(ta_values[chain42161_idx])


def test_stamped_tvl_feeds_period_metrics():
    """Verify stamped total_assets reaches period metrics tvl_end for ranking.

    ForgeYields has valid share_price but NaN total_assets. After stamping
    total_assets on the last row, the period metrics pipeline should see
    a non-null tvl_end and the vault passes ranking filters.

    1. Build a DataFrame with valid share_price, NaN total_assets except last row
    2. Run calculate_period_metrics
    3. Assert tvl_end equals the stamped value
    """
    from eth_defi.research.vault_metrics import calculate_period_metrics
    from eth_defi.vault.fee import FeeData, VaultFeeMode

    # 1. Build price data: 30 days of hourly rows
    ...
    # Stamp total_assets on the last row (simulating stamp_external_tvl)
    prices_df.iloc[-1, prices_df.columns.get_loc("total_assets")] = 1_085_984.0
    ...
    pm = calculate_period_metrics(
        ...
        tvl=prices_df["total_assets"],
        now_=dates[-1],
    )

    # 3. Assert tvl_end is populated from the stamped total_assets
