...
AERA_USDC_UNDERLYING_VAULT: HexAddress = "0xFA60E843a52eff94901f43ac08232b59351192cc"

#: Aera V2 documented maximum raw TVL fee per second.
AERA_MAX_RAW_TVL_FEE_PER_SECOND = 10**9

#: Annualised Aera maximum TVL fee, mapped to management fee.
EXPECTED_AERA_MAX_MANAGEMENT_FEE = 0.031536

#: Current Aera V2 TVL fee on the tested strategy, mapped to annual management fee.
EXPECTED_AERA_MANAGEMENT_FEE = 0.0
...
AERA_VAULT_ADDRESSES: set[HexAddress] = {
...
def test_aera_hardcoded_addresses() -> None:
    ...
    1. Iterate through the known Aera address set
    2. Look up each address in hardcoded protocol classification
    3. Verify each address maps to the Aera feature flag
    """
    # 1. Iterate through the known Aera address set
    for address in AERA_VAULT_ADDRESSES:
        # 2. Look up each address in hardcoded protocol classification
        features = HARDCODED_PROTOCOLS[address]

        # 3. Verify each address maps to the Aera feature flag
        assert features == {ERC4626Feature.aera_like}


def test_aera_tvl_fee_annualisation() -> None:
    """Check Aera TVL fee annualisation maths with a non-zero input.

    1. Use Aera V2's documented maximum per-second TVL fee
    2. Convert the raw fee to an annual management fee
    3. Verify the annualised management fee is 3.1536%
    """
    # 1. Use Aera V2's documented maximum per-second TVL fee
    raw_fee_per_second = AERA_MAX_RAW_TVL_FEE_PER_SECOND

    # 2. Convert the raw fee to an annual management fee
    management_fee = convert_aera_tvl_fee_to_annual_management_fee(raw_fee_per_second)

    # 3. Verify the annualised management fee is 3.1536%
    assert management_fee == pytest.approx(EXPECTED_AERA_MAX_MANAGEMENT_FEE)
...
def test_aera(web3: Web3) -> None:
    ...
    1. Detect Aera feature flags from the live fork
    2. Create the typed Aera vault adapter
    3. Verify protocol metadata, fees, risk and app link
    """
    # 1. Detect Aera feature flags from the live fork
    ...
    assert vault.get_management_fee("latest") == pytest.approx(EXPECTED_AERA_MANAGEMENT_FEE)
    assert vault.get_performance_fee("latest") == pytest.approx(EXPECTED_AERA_PERFORMANCE_FEE)
...
def test_aera_strategy_fees(web3: Web3) -> None:
    ...
    1. Create the typed Aera vault adapter
    2. Read the underlying Aera V2 vault address
    3. Verify TVL/performance fees and fee accounting mode
    """
    # 1. Create the typed Aera vault adapter
    vault = create_vault_instance_autodetect(
        web3,
