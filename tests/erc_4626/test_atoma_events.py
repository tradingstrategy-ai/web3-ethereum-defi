...
def test_atoma_withdraw_topics_are_withdrawals() -> None:
    """Verify Atoma's async withdrawal events are counted as redemptions."""
    web3 = Web3()
    atoma_events = get_atoma_vault_discovery_events(web3)
    topic_map = get_vault_event_topic_map(web3)

    withdrawal_requested_topic = get_topic_signature_from_event(atoma_events[0])
    withdrawal_claimed_topic = get_topic_signature_from_event(atoma_events[1])
...
def test_atoma_topics_are_in_aggregate_discovery_events() -> None:
    """Verify aggregate discovery scans include all Atoma custom topics."""
    web3 = Web3()
