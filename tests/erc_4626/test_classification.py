...
def test_ptokens_are_address_scoped_to_robinhood_chain() -> None:
    """Classify reviewed pTokens without using Arcus infrastructure as a signal."""
    ...
    for address in (PTOKEN_BTC_3X_LONG_VAULT, PTOKEN_HOOD_3X_LONG_VAULT):
        assert _get_hardcoded_protocol_features(address, chain_id=ROBINHOOD) == expected
        assert _get_hardcoded_protocol_features(address, chain_id=BASE) is None

    assert "bridgeVault" not in CHAIN_RESTRICTED_PROBES
