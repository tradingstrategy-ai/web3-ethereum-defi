...
def test_build_raw_prices_low_leader_fraction_is_capacity_not_closure(tmp_path):
...
def test_low_share_cap_not_carried_into_unobserved_row(tmp_path):
...
def test_existing_metrics_database_accepts_unknown_deposit_flags(tmp_path):
    """Reopen an older scanner schema without inventing open permission.

    1. Recreate the former non-null deposit-flag constraints.
    ...
    # 1. Recreate the former non-null deposit-flag constraints.
    db = HyperliquidDailyMetricsDatabase(path)
