...
def test_swap_revert_with_slippage_protection(
    ...
    usdc.functions.transfer(fund_client, usdc_amount_to_pay).transact({"from": deployer})
    usdc.functions.approve(router.address, usdc_amount_to_pay).transact({"from": fund_client})
...
def test_swap_three_way_revert(
    ...
    # give user_2 some cash as well
    weth_amount = 1 * 10**18
    weth.functions.transfer(fund_client, weth_amount).transact({"from": deployer})
    weth.functions.approve(router.address, weth_amount).transact({"from": fund_client})
