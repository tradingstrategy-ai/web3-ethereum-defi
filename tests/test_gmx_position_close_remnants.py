...
def test_full_close_uses_exact_gmx_position_size(mock_fetch_erc20, mock_get_positions, mock_extract_order_key, mock_extract_execution, mock_gmx_exchange, mock_gmx_position_with_fees):
    ...
    mock_positions_manager.get_data.return_value = {"ASTER_short": mock_gmx_position_with_fees}
    ...
    with patch.object(mock_gmx_exchange, "fetch_ticker") as mock_ticker:
        ...
        with patch.object(mock_gmx_exchange, "trader") as mock_trader:
            ...
            assert call_kwargs["size_delta_usd"] == 9.85, f"Expected size_delta_usd=9.85 (exact GMX), got {call_kwargs['size_delta_usd']}"

            # Verify it's not using calculated value
            assert call_kwargs["size_delta_usd"] != 9.90, "Should NOT use calculated amount×price (9.90), should use GMX exact (9.85)"
...
@patch("eth_defi.gmx.ccxt.exchange.GetOpenPositions")
@patch("eth_defi.gmx.ccxt.exchange.fetch_erc20_details")
def test_partial_close_clamps_to_actual_position(mock_fetch_erc20, mock_get_positions, mock_extract_order_key, mock_extract_execution, mock_gmx_exchange, mock_gmx_position_remnant):
    ...
    mock_positions_manager = Mock()
    mock_positions_manager.get_data.return_value = {"ASTER_short": mock_gmx_position_remnant}
    mock_get_positions.return_value = mock_positions_manager
    ...
    with patch.object(mock_gmx_exchange, "fetch_ticker") as mock_ticker:
        ...
        with patch.object(mock_gmx_exchange, "trader") as mock_trader:
            ...
            assert call_kwargs["size_delta_usd"] == 1.86, f"Should clamp to actual position $1.86, got {call_kwargs['size_delta_usd']}"
...
def test_close_with_no_position_returns_synthetic_order(mock_get_positions, mock_fetch_erc20, mock_gmx_exchange):
    """Test that attempting to close a non-existent position returns synthetic "already closed" order instead of error.

    This handles the race condition where stop-loss closed position
    before main trading loop attempts to close it.
    """
    # Disable gas monitoring
    mock_gmx_exchange._gas_monitor_config = None
    ...
    with patch.object(mock_gmx_exchange, "fetch_ticker") as mock_ticker:
        ...
        # Execute: Try to close non-existent position
        order = mock_gmx_exchange.create_order(
            symbol="ASTER/USDC:USDC",
            type="market",
            side="buy",
        ...
        # Assert: Synthetic order returned
