...
def depositor(web3, usdt, usdt_holder) -> HexAddress:
    ...
    tx_hash = usdt.contract.functions.transfer(address, 999 * 10**6).transact({"from": usdt_holder, "gas": 100_000})
    assert_transaction_success_with_explanation(web3, tx_hash)
    return address


def test_lagoon_deploy_binance_guarded_any_token(
    web3: Web3,
    uniswap_v2,
    wbnb: TokenDetails,
    usdt: TokenDetails,
...
def test_lagoon_deploy_from_scratch_binance_guarded_any_token(
    ...
    deploy_info = deploy_automated_lagoon_vault(
        ...
        uniswap_v3=None,
        any_asset=True,
        from_the_scratch=True,  # Need to deploy a custom fee registry
    )
