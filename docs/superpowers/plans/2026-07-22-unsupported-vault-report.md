...
- Extend `VaultFlowError` on the `origin/master` baseline with optional `function_selector: HexBytes`, `error_selector: HexBytes`, and `access_delay: int` fields before using them from a manager. Store them as structured attributes and include them in `__str__()` diagnostics; retain backward compatibility for existing callers. Keep the denied entry point separate from an expected or decoded custom error so downstream diagnostics do not conflate the two selectors. The baseline has no exception serialiser, so do not invent a second report schema solely for this change; if implementation discovers a consumer that serialises `VaultFlowError`, extend that representation with all three fields. Add `test_vault_flow_unavailable_preserves_access_context` to `tests/vault/test_deposit_redeem.py` to cover construction, attribute retention, and exact string formatting, plus the existing serialiser if one is found.
- Before creating a deposit, call the existing access-manager `canCall(caller, vault, deposit(uint256,address).selector)`. Convert a false result or non-zero delay into `VaultFlowUnavailable` containing the caller, `function_selector`, and `access_delay`. Populate `decoded_error` only when a revert was actually decoded; `canCall()` returning a delay is policy metadata, not a decoded revert. Distinguish a caller that is never permitted from one that is permitted only after an access-manager scheduling delay; the synchronous manager rejects both, but the latter must remain discoverable for a future scheduled-execution flow. Preserve unexpected RPC/contract errors rather than classifying them as private.
...
source .local-test.env && poetry run pytest tests/erc_4626/test_scan_features.py::test_create_vault_scan_record_persists_deposit_permission tests/research/test_vault_metrics.py::test_calculate_lifetime_metrics_exports_deposit_permission tests/research/test_vault_metrics.py::test_calculate_lifetime_metrics_exports_permission_for_refusing_manager tests/research/test_vault_metrics.py::test_calculate_lifetime_metrics_defaults_legacy_deposit_permission_to_unknown tests/research/test_vault_metrics.py::test_calculate_lifetime_metrics_preserves_null_deposit_manager -v
