...
- Hibachi
- ApeX Omni
- Pacifica lakes, which are not yet wired into the shared scanner
...
| Pacifica | Public `GET /lake/list` and `GET /account`: `last_checked_equity`/`account_equity` | Public `GET /positions`; `GET /info/prices` supplies current marks | Position `symbol`, `side`, `amount` plus same-snapshot mark price and timestamp. Margin, isolation and liquidation fields are excluded | Account and positions after adding a native reader |
...
Every row below uses the same observation dataclasses, three common DuckDB tables,
derivation helper, temporal join and seven cleaned-Parquet columns. The only
protocol-specific code is the API adapter and its declarative identity/sign
mapping.
...
| Protocol | Collect in adapter | Store | Pass to cleaned Parquet |
|---|---|---|---|
...
| Lighter | Read the public-pool `/account` response; map account index/deployment, `total_asset_value`, market, `sign`, non-zero `position` and absolute `position_value` | Common tables in `LighterDailyMetricsDatabase` | Join to `build_lighter_prices_dataframe()` rows using shared chain `9998` and the deployment-specific synthetic address generated by the adapter |
| Pacifica | Read `/lake/list`, `/account`, `/positions` and one batched `/info/prices`; map side/amount and contemporaneous mark | Common tables in the new `PacificaMetricsDatabase` | Join to the new Pacifica raw price DataFrame using reserved synthetic chain `9994` and the canonical lower-case/base58 lake address |
...
The adapter must produce `dataset_chain_id` and `dataset_address` using the
...
1. an adapter returning the common observation bundle;
...
protocol API
  ...
  -> generic backward as-of join to native raw price rows
...
Protocol-specific code ends at the adapter. Storage, aggregation, temporal
...
|---|---|
| `adapter.py` | `PerpVaultAccountAdapter` protocol and declarative capability/identity mapping |
...
promote it to float, and must cast `perp_metrics_observed_at` explicitly to
Arrow `timestamp[ms]`. It rejects timezone-aware values, unsafe precision loss,
NaN/infinite monetary values and status values outside
`PerpParquetDataStatus`; Pandas' inferred datetime unit is never the contract.

The adapter returns `PerpVaultObservationBundle(account, positions)`. It owns
only source endpoint calls and field/sign conversion. The common collector
validates the bundle and writes it atomically through
`write_perp_vault_observation_bundle()`. A failed or shortened position read
must not commit an `available` bundle.

`collect_and_store_perp_vault_observations()` accepts an adapter, the vault
identities already discovered by the native scanner, its observation store and
`max_workers`. It uses `joblib.Parallel` with the threading backend for network
reads, then validates and writes completed bundles on the database owner
thread. Every command-line scanner exposes the same `MAX_WORKERS` setting.
Protocol scan orchestration only registers the adapter; it does not implement
its own loop, retry policy, aggregation or writes.
...
| `perp_metrics_observed_at` | `timestamp[ms]` | Position-effective time, or collector time when the position source has none |
...
`not_public`, `authentication_required`, `source_error`, `not_implemented`,
`not_collected` or `stale`, all four numeric columns are null. Registered perp
vault rows before their first observation use `not_collected`; non-perp vault
rows use `not_applicable`.
...
4. performs a backward-only `merge_asof`, with
   `by=["chain", "address"]`, from each price timestamp to the
   latest position-effective timestamp with no age tolerance;
5. never attaches a snapshot to a price row earlier than that snapshot;
6. validates matching quote assets;
7. restores the original price-row order after the join and asserts unchanged
   row count and unique price-row identity;
8. leaves status and metric values null only when no earlier observation
   exists, rather than deciding default status or backfilling.

The join is deliberately age-blind: it preserves the latest candidate's
values, quote asset and observation time even when old. It never applies the
maximum-age policy. `finalise_perp_metric_columns()` is the sole freshness
owner and converts an old `available` candidate to `stale`.
...
time. A negative age is a join invariant violation. The boundary is shared by
raw-to-cleaned processing and `forward_fill_vault()`.
...
- for an `available` row older than the global maximum age, change status to
  `stale` and null long, short, count and largest position;
- retain `perp_quote_asset` and `perp_metrics_observed_at` on stale rows so
  consumers can diagnose denomination and age.
...
If the latest account snapshot is newer than every available price row, do not
attach it to an older row. It remains in DuckDB until a subsequent price row
has an equal or later timestamp. This avoids future information leaking into
...
producing non-standard JSON. `observed_at` is a millisecond-precision naive UTC
ISO-8601 string. These rules belong to the generic exporter and apply equally
to every protocol.
...
{
  "schema_version": 1,
  "observed_at": "2026-07-24T12:00:00.000",
  "quote_asset": "USDC",
...
- Capture redacted live fixtures for an active and empty vault for
  Hyperliquid, Lighter and Pacifica.
...
- Freeze the seven cleaned-Parquet column names/types and the generic
  backward-only join/maximum-age policy.
...
- Add `eth_defi/perp_dex/{adapter,metrics,storage,parquet,export}.py` and its
  Sphinx API stub.
- Add the shared dataclasses, two status enums, adapter protocol, validators,
...
- Add `finalise_perp_metric_columns()` once at the end of cleaned-data
  ...
  assigns default/stale statuses and nulls expired position values.
...
### Phase 4: Pacifica native reader and adapter

- Add vault discovery from `/lake/list` and a stable synthetic chain/deployment
  identity.
- Reserve `PACIFICA_CHAIN_ID = 9994` in
  `eth_defi/pacifica/constants.py`; add it to `CHAIN_NAMES`, chain metadata,
  identity fixtures and protocol documentation. Treat this as a dataset
  namespace, not a claim that Pacifica is an EVM chain.
- Fetch `/account`, `/positions` and `/info/prices`.
- Map equity plus signed quantity/side and current mark into the common
  observations.
- Batch the one global price read across vaults and enforce timestamp-skew and
  rate-limit controls.
- Ignore account margin, position margin, isolation and liquidation fields.
- Write through the common bundle helper and register its raw price frame with
  the existing native merge; all exposure handling remains generic.
...
- In every GRVT, Hibachi and ApeX README listed in phase 5, state:
  ...
  - the verification date and links to the relevant official API/privacy
    documentation.
- Roll out Lighter, Hyperliquid, Pacifica, GRVT, Hibachi and ApeX in that order.
...
- Test exact-time and backward as-of joins, a snapshot one millisecond newer
  than a price row, the maximum-age boundary, stale nulling, rows before the
  first snapshot and mixed protocol/deployment identities.
...
- Test that both raw-to-cleaned forward fills and
  ...
  stale rows retain quote asset/observation time but no position numbers.
...
- Test `normalise_perp_metric_parquet_dtypes()` produces exact Arrow
  `timestamp[ms]` and nullable `int64` fields from supported Pandas/Arrow
  inputs and rejects timezone-aware timestamps or unsafe precision loss.
...
- Assert final JSON values equal calculations from the latest cleaned row and
  ...
  null, timestamps have millisecond precision and NaN/infinity are rejected.
