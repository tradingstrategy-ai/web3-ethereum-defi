...
3. **Split the Foundry cache: immutable toolchain vs one stable accumulating RPC
   ...
   static key `foundry-v1.5.0-${{ runner.os }}`, so the toolchain and the fork RPC
   ...
   `foundry-toolchain-v1.5.0-*` key. (`foundry-rs/foundry-toolchain` also caches
   ...
   - The fork RPC cache is **one stable, self-warming cache** — the simplest
     ...
     `foundry-rpc-v1.5.0-${{ runner.os }}-${{ github.run_id }}` with a **stable
     restore-keys prefix** `foundry-rpc-v1.5.0-${{ runner.os }}-`, split into
   ...
   - Anvil writes fork reads under `~/.foundry/cache/rpc/`. The observed layout is
     ...
     sub-path, and **re-validate the exact layout against pinned Foundry v1.5.0**
