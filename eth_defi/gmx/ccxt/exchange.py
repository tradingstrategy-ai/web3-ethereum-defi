...
class GMX(ExchangeCompatible):
    ...
    **GMX Limitations:**
    ...
    def _get_bulk_market_infos(self, ttl: int = 60) -> dict:
        ...
        try:
            all_infos = self.subsquid.get_market_infos(limit=200)
            by_addr: dict = {}
            for mi in all_infos:
        ...
        if not reduceOnly:
            ...
            try:
                ...
                if not position_to_close:
                    ...
                    logger.warning(
                        "No %s position found for %s with collateral %s. This likely means the position was already closed (e.g., by stop-loss execution). Returning synthetic 'closed' order response.",
                        position_type,
                        symbol,
                ...
                if (
                    ...
                    is_long = (
                        side != "sell"
                    )  # we were trying to close a long (sell side)
                    ...
                    if market_address and hasattr(self, "subsquid") and self.subsquid:
                        try:
                            ...
                            logger.warning(
                                "Subsquid lookup failed for %s: %s", symbol, sq_err
                            )
