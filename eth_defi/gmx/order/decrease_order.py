...
class DecreaseOrder(BaseOrder):
    ...
    def create_decrease_order(
        ...
        Creates an unsigned transaction for closing or reducing a position on GMX.
        The transaction needs to be signed and sent by the user.

        :param size_delta: Position size to decrease in USD
        :type size_delta: float
        :param initial_collateral_delta_amount: Amount of collateral to remove (in token's smallest unit)
