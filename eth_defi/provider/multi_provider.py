...
def create_multi_provider_web3(
    ...
    urls: Set[ParseResult] = set()
    for parsable in items:

        parsable = parsable.strip()

        try:
            url = urlparse(parsable)
    ...
    transact_endpoints = [url for url in urls if url.scheme.startswith("mev+")]
    call_endpoints = [url for url in urls if not url.scheme.startswith("mev+")]
    ...
    call_providers = [HTTPProvider(str(url)) for url in call_endpoints]
    ...
    if len(transact_endpoints) > 0:
        transact_endpoint = str(transact_endpoints[0])
        transact_provider = HTTPProvider(transact_endpoint)

        patch_provider(transact_provider)
    ...
    web3 = MultiProviderWeb(provider)
