...
def _calculate_compound_interest_multiplier(rate: Decimal, seconds: Decimal | float) -> Decimal:
    ...
    Based on https://github.com/aave/aave-v3-core/blob/master/contracts/protocol/libraries/math/MathUtils.sol#L51
    """
...
def aave_v3_calculate_accrued_interests(df: DataFrame, start_time: datetime, end_time: datetime, amount: Decimal, token: str = '') -> Tuple[datetime, datetime, Decimal, Decimal, Decimal]:
    """
    ...
    # Based on balanceOf() https://github.com/aave/aave-v3-core/blob/master/contracts/protocol/tokenization/AToken.sol#L131
    ...
    # Based on https://github.com/aave/aave-v3-core/blob/master/contracts/protocol/tokenization/VariableDebtToken.sol#L78
    ...
    # Based on balanceOf() https://github.com/aave/aave-v3-core/blob/master/contracts/protocol/tokenization/StableDebtToken.sol#L102
    ...
    return actual_start_time, actual_end_time, deposit_interest, variable_borrow_interest, stable_borrow_interest


# Simplified shortcut functions for calculating accrued interest

def aave_v3_calculate_accrued_deposit_interest(df: DataFrame, start_time: datetime, end_time: datetime, amount: Decimal, token: str = '') -> Tuple[datetime, datetime, Decimal]:
    result = aave_v3_calculate_accrued_interests(df, start_time, end_time, amount, token)
    return result[0], result[1], result[2]


def aave_v3_calculate_accrued_variable_borrow_interest(df: DataFrame, start_time: datetime, end_time: datetime, amount: Decimal, token: str = '') -> Tuple[datetime, datetime, Decimal]:
    result = aave_v3_calculate_accrued_interests(df, start_time, end_time, amount, token)
    return result[0], result[1], result[3]


def aave_v3_calculate_accrued_stable_borrow_interest(df: DataFrame, start_time: datetime, end_time: datetime, amount: Decimal, token: str = '') -> Tuple[datetime, datetime, Decimal]:
    result = aave_v3_calculate_accrued_interests(df, start_time, end_time, amount, token)
