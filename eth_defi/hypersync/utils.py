...
from eth_defi.hypersync.session import ThrottledHypersyncClient, _create_limiter, create_throttled_hypersync_client, get_hypersync_concurrency_from_env, get_hypersync_rpm_from_env
...
def configure_hypersync_from_env(
    ...
    :param web3:
        Web3 connection or EVM chain ID used to select the HyperSync endpoint.
    ...
    if scan_backend == "auto":
        assert hypersync_api_key, "HYPERSYNC_API_KEY must be set to use auto scan backend"
    ...
    match scan_backend:
        ...
        case "hypersync":
            ...
            chain_id = web3 if isinstance(web3, int) else web3.eth.chain_id
            assert hypersync_url, f"No HyperSync server available for chain {chain_id}"
            assert hypersync_api_key, "HYPERSYNC_API_KEY must be set to use HyperSync backend"
            hypersync_client = _make_client(hypersync_url)
        case "rpc":
            hypersync_client = None
