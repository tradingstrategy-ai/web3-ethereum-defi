...
def deploy_fiat_token(
    ...
    # v1 init
    token_contract.functions.initialize(
        token_name,
        token_symbol,
        token_currency,
        decimals,
    ...
    token_contract.functions.initializeV2(
        token_name,
    ).transact({
        "from": deployer,
    })

    token_contract.functions.initializeV2_1(
        deployer,
    ).transact({
        "from": deployer,
    })

    token_contract.functions.configureMinter(
        deployer,
