...
from eth_defi.tokenised_fund.libeara.constants import LIBEARA_PRODUCTS, LIBEARA_ULTRA_ARBITRUM, LibearaProduct
...
def create_detection(product: LibearaProduct = LIBEARA_ULTRA_ARBITRUM) -> ERC4262VaultDetection:
...
def create_lead(product: LibearaProduct = LIBEARA_ULTRA_ARBITRUM) -> PotentialVaultMatch:
...
def upsert_ultra_metadata(vault_db: VaultDatabase, row: dict, product: LibearaProduct = LIBEARA_ULTRA_ARBITRUM) -> None:
    ...
    vault_db.leads[spec] = create_lead(product)
...
def main() -> None:
    ...
    products = tuple(product for product in LIBEARA_PRODUCTS.values() if product.symbol == "ULTRA")
    logger.info(
        ...
        ",".join(f"{product.chain_id}:{product.token}" for product in products),
        "ULTRA",
        min(product.first_seen_at_block for product in products),
        dry_run,
    )
    if dry_run:
        return
    ...
    for product in products:
        ...
        if end_block < product.first_seen_at_block:
            raise ValueError(f"END_BLOCK must be at least {product.first_seen_at_block} for {product.token}")
        row = create_vault_scan_record(web3, detection=create_detection(product), block_identifier=end_block, token_cache=TokenDiskCache())
        upsert_ultra_metadata(vault_db, row, product)
